 ...
 * only solution is to use unix socket sendmsg, SCM_RIGHTS(sebd ir receuve a set of open file descriptors from another process.), msg_control
 ...
 * - POSIX already require that two processes be able to share the same underlying
 * reference to a file because of the semantics of the fork call. Adding some ability
 * to share arbitrary file descriptors between two processes then is far more about
 * how you ask the kernel than the actual file descriptor sharing operating.
 * In linux, file descriptors can be passed through local network sockets. The
 * sender constructs a mystic-looking sendmsg call, placing the file descriptor
 * in the control field of that operation. The kernel pulls the file descriptor out of 
 * the control field, allocates a file descriptor in the target process which references 
 * the same file object and then sticks the file descriptor in a queue for the receiving 
 * process to fetch
